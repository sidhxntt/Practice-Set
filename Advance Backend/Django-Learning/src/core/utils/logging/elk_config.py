...
ES_INDEX_PREFIX = getattr(settings, 'ELASTICSEARCH_INDEX_PREFIX', 'django')

# Logstash configuration
LOGSTASH_HOST = getattr(settings, 'LOGSTASH_HOST', 'localhost')
LOGSTASH_PORT = getattr(settings, 'LOGSTASH_PORT', 5050)
...
class LogstashHandler(logging.handlers.SocketHandler):
    """
    Log handler that sends logs to Logstash via TCP.
    """
    ...
    def emit(self, record):
        try:
            ...
            msg += '\n'  # Add newline for Logstash TCP input
...
def configure_logging():
    ...
    if USE_LOGSTASH:
        LOGGING['handlers']['logstash'] = {
            'level': 'INFO',
            'class': 'core.logging.elk_config.LogstashHandler',
            'host': LOGSTASH_HOST,
            'port': LOGSTASH_PORT,
        }
        
        # Add logstash handler to each logger
