...
def album_pre_delete_handler(sender, instance, **kwargs):
    """
    Log before album deletion
    """
    print(f'About to delete album: {instance.album_info()}')
    
    # Check if album has photos and handle accordingly
    if instance.photos.exists():
        print(f'Album contains {instance.photos.count()} photos that will be deleted')
